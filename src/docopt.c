...
"Usage:\n"
"  dq info <queue_file>\n"
"  dq empty <queue_file> [-a=<num> -b=<test>]\n"
"  dq <queue_file> -c\n"
"  dq --help\n"
"\n"
"Options:\n"
...
//            command->value = true;
            command->value = strdup(ts->current);
            tokens_move(ts);
            return 0;
        }
    }
...
int elems_to_args(Elements *elements, DocoptArgs *args, bool help,
    ...
    for (i=0; i < elements->n_options; i++) {
        ...
        if (help && option->value && !strcmp(option->olong, "--help")) {
            ...
            printf("%s\n", version);
            return 1;
        } else if (!strcmp(option->olong, "--ccc")) {
            args->ccc = option->value;
        } else if (!strcmp(option->olong, "--help")) {
            args->help = option->value;
        } else if (!strcmp(option->olong, "--append")) {
            if (option->argument)
                args->append = option->argument;
        } else if (!strcmp(option->olong, "--bbb")) {
            if (option->argument)
                args->bbb = option->argument;
        }
    }
    /* commands */
    for (i=0; i < elements->n_commands; i++) {
        command = &elements->commands[i];
        if (!strcmp(command->name, "empty")) {
            args->empty = command->value;
        } else if (!strcmp(command->name, "info")) {
            args->info = command->value;
        }
    }
    /* arguments */
    for (i=0; i < elements->n_arguments; i++) {
        argument = &elements->arguments[i];
        if (!strcmp(argument->name, "<queue_file>")) {
            args->queue_file = argument->value;
        }
    }
    return 0;
}
...
DocoptArgs docopt(int argc, char *argv[], bool help, const char *version) {
    ...
    Elements elements = {2, 1, 4, commands, arguments, options};
